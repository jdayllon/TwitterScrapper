...
def scape_twitter_by_date(query, start_date=arrow.get().format('YYYY-MM-DD'), end_date=arrow.get().shift(years=-10).format('YYYY-MM-DD')):
    """Simple program that greets NAME for a total of COUNT times."""

    cur_date = arrow.get(start_date) 
    finish_date = arrow.get(end_date) 

    print("Scrapping twitter with:[%s]\nFrom Date:[%s]\nTo Date:[%s]" % (query, start_date.format('YYYY-MM-DD'), end_date.format('YYYY-MM-DD')))
    ...
    print("Num requests to send: %d" % len(urls))
